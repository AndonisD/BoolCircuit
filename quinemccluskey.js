...
export default class QuineMcCluskey {
	...
	getEssentialPrimeImplicants() {
		...
		let uncoveredTerms = Object.keys(this.piTable).map((str) => {
			return parseInt(str);
		});

		let remainingPrimeImplicants = this.getRemainingPrimeImplicants(
			essentialPrimeImplicants
		);

		let minPrimeImpSet = this.coverUnusedTerms(
	...
	coverUnusedTerms(remainingPrimeImplicants, uncoveredTerms) {
		...
		let allCombinations = getAllSubsets(remainingPrimeImplicants).sort(
			(a, b) => {
